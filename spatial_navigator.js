...
(function(exports) {
  ...
  SpatialNavigator.prototype = evt({
    ...
    isFocusableElement: function snIsFocusableElement(elem) {
    ...
    focusSilently: function snFocusSliently(elem) {
      ...
      if (!elem) {
        elem = this._getAllRects()[0].element;
      } else if (this._collection.indexOf(elem) < 0 ||
                 !this.isFocusableElement(elem)) {
        console.error('Focus Failed!');
        console.error('Current focus element: ', elem, ' is not in the valid state.');
        console.error('Please make sure the element is visible on the screen!');
        return false;
      }
